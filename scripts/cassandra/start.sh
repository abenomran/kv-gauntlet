...
draw_ui() {
  ...
  # Header
  printf "\033[K${CYAN}Waiting for Cassandra cluster to become ready...${RESET}\n"
  ((lines++))

  # Summary (NOT dim anymore)
...
main() {
  ...
  while true; do
    ...
    if (( elapsed >= MAX_WAIT_SECONDS )); then
      printf "\n${RED}✖ ERROR: Cluster did not become ready after %ss.${RESET}\n" "$MAX_WAIT_SECONDS"
      exit 1
    fi
